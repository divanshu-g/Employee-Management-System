...
async function updateRole(req, res, next) {
  ...
  const allowed = ALLOWED_ROLES.some(r => userRoles.includes(r));
  if (!allowed) {
    return res.status(403).json({ message: "Forbidden: Not authorized to update roles." });
  }
  const role_id = parseInt(req.params.id, 10);
  const { role_name, role_type, role_description, permissions, is_active } = req.body;
  try {
    const updatedRole = await prisma.role.update({
      ...
      data: {
        ...(role_name ? { role_name } : {}),
        ...(role_type ? { role_type } : {}),
        role_description: role_description !== undefined ? role_description : undefined,
...
async function deleteRole(req, res, next) {
  ...
  const allowed = ALLOWED_ROLES.some(r => userRoles.includes(r));
  if (!allowed) {
    return res.status(403).json({ message: "Forbidden: Not authorized to delete roles." });
  }
  const role_id = parseInt(req.params.id, 10);
  try {
    // Soft delete
